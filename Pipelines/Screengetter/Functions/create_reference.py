...
## HELPER FUNCTIONS ##
def _determine_smallest_resolution(
    files: Iterable[Path],
    output_dir: Path,
...
def create_reference(
    ...
    *,
    ...
    ----------
    input_dir:
        Path to directory with videos to sample and use as reference set.

    output_dir:
        Path to directory to store reference set.

    similarity_threshold:
        Similarity threshold to determine when a new frame is to be kept as part of the reference set or discarded.
    
    seed_use:
        Random seed to ensure same sampling of videos for reference.

    Returns
    -------
    List[dict] of reference set.
    dict with smallest resolution {"width", "height"}
    """
    ...
    output_dir.mkdir(parents = True, exist_ok = True)
    smallest_resolution = _determine_smallest_resolution(
        files = video_files,
